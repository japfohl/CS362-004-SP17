...
void randomTestVillage(unsigned long count) {
	...
	for (i = 0; i < count; ++i) {
		...
		/*Give the player a Village; randomizeGameState ensures the current player
		...
		int shuffleFlag = snapshot.deckCount[player] - 1;
		if (shuffleFlag < 0)
			shuffleFlag = 1;
		else
			shuffleFlag = 0;
		...
		if (state.playedCards[state.playedCardCount - 1] != village) {
			addError(&t,"Error: Last played card is not village");
		}
		...
		//Check deck
		if (shuffleFlag == 0) {
			if (state.deckCount[player] > snapshot.deckCount[player] - 1) {
		...
		//Check actions
		if (state.numActions < snapshot.numActions + 2) {
			addError(&t,"Error: Not enough actions.");
		}
		if (state.numActions > snapshot.numActions + 2) {
			addError(&t,"Error: Too many actions.");
		}
